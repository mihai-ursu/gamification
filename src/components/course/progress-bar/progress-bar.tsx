...
const ProgressBar = ({ noOfQuestions }: { noOfQuestions: number }) => {
  const { correctQuestions } = useQuestionsStore();
  const percentage = Math.round((correctQuestions / noOfQuestions) * 100);
  ...
  return (
    <div className="relative h-5 w-full overflow-hidden rounded-full bg-slate-200">
      <div
        ...
        className={cn(
          "absolute left-0 top-0 h-full rounded-full bg-orange-400 transition-all duration-300",
          correctQuestions === 0 ? "opacity-0" : "opacity-100",
        )}
      >
